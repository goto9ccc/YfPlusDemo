  ...
  try
    ...
    try
      ...
      begin
        ...
        Grid.DrawTreeArea(FCanvas, Rect(0, 0, Width, Height), False);
        if Grid.UseRightToLeftAlignment then
...
//    (ARow <> RowCount - FooterRowCount - 1 - TopDataOffset))) and
  ...
  if not Result then
    Result := Result or ((dghFooter3D in OptionsEh) and
      (FooterRowCount > 0) and (ARow > RowCount - TopDataOffset - 1 {- FooterRowCount - 1 - TopDataOffset}));
  if not Result then
    Result := Result or ((dghData3D in OptionsEh) and not (gdFixed in AState)
      and not ((FooterRowCount > 0) and (ARow > RowCount - TopDataOffset - 1 {RowCount - FooterRowCount - 1 - TopDataOffset})));
{  if not Result then
...
//      else NewRow := FDataLink.DataSet.RecNo - 1 + TopDataOffset;
...
procedure TCustomDBGridEh.DrawTreeArea(Canvas: TCanvas; ARect: TRect; IsDrawEdge: Boolean);
var
  ARect1: TRect;
  i: Integer;
  OneAdd: Integer;
  ...
  begin
    ...
    DrawTreeElement(Canvas, ARectE, TreeElement, //False);
      (DefaultRowHeight mod 2 = 1) and (TopRow mod 2 = 1), 1, 1, UseRightToLeftAlignment);
  end;
...
begin
  ...
  begin
    ...
    DrawTreeArea(Canvas, ARect, True);
...
begin
  if DBGridEhDataService <> nil then
    DBGridEhDataService.ExecuteFindDialog(Self, Text, FieldName, Modal)
  else
...
begin
  Result := DBGridEhDataService.LocateText(Self, FieldName, Text,
...
begin
  ...
  begin
    RecordFounded := LocateText(Self, '', NesSs,
      [ltoCaseInsensitiveEh], InpsDirection, ltmFromBegingEh, lttInAllNodesEh);
...
function TDBGridEhDataService.LocateText(AGrid: TCustomDBGridEh; const FieldName: string;
  const Text: String; Options: TLocateTextOptionsEh; Direction: TLocateTextDirectionEh;
  Matching: TLocateTextMatchingEh; TreeFindRange: TLocateTextTreeFindRangeEh): Boolean;
var
  DatasetFeatures: TDatasetFeaturesEh;
...
function TDBGridEhDataService.DefaultLocateText(AGrid: TCustomDBGridEh; const FieldName: string;
  const Text: String; Options: TLocateTextOptionsEh; Direction: TLocateTextDirectionEh;
  Matching: TLocateTextMatchingEh; TreeFindRange: TLocateTextTreeFindRangeEh): Boolean;
begin
...
procedure TDBGridEhDataService.ExecuteFindDialog(AGrid: TCustomDBGridEh; Text, FieldName: String; Modal: Boolean);
var
  DatasetFeatures: TDatasetFeaturesEh;
begin
  if (AGrid.DataSource <> nil) and (AGrid.DataSource.DataSet <> nil) then
