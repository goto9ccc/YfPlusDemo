...
type
  TFormInvmc = class(TBaseSearchForm)
    edtMB003: TEdit;
    lbl1: TLabel;
    lbl2: TLabel;
    edtMC002: TEdit;
...
var
   FormInvmc: TFormInvmc;

implementation

uses Frm_OpenSearch, Invmc_Module;


{$R *.dfm}

procedure TFormInvmc.btnMC002Click(Sender: TObject);
VAR
  sSQL:String;
begin
  inherited;
    sSQL := 'SELECT MC001 仓库编号,MC002 仓库名称 from CMSMC Where 1 = 1 ';
    FormOpenSearch := TFormOpenSearch.Create(nil,sSQL,'仓库编号','仓库名称','MC001','MC002');
    FormOpenSearch.Caption := '仓库查询';
    FormOpenSearch.dbgrdh.Font.Size := 12;
    FormOpenSearch.qry.Open;
    FormOpenSearch.ShowModal;
    edtMC002.Text := Trim(FormOpenSearch.resultOne);
    lblMC002.Caption := FormOpenSearch.resultTwo;
    FormOpenSearch.Free;
end;

procedure TFormInvmc.btnsearchClick(Sender: TObject);
begin
  inherited;
    qry.Close;
...
procedure TFormInvmc.dbgrdhDblClick(Sender: TObject);
var
  NonDeliverySum :Currency;
begin
  inherited;
  Invmc_DataModule := TInvmc_DataModule.Create(Application);
  NonDeliverySum  := Invmc_DataModule.getNonDeliverySum(qry.FieldByName('品号').AsString);
  edtys.Text := CurrToStr(NonDeliverySum);
  edtky.Text := CurrToStr(Qry.FieldByName('库存数量').AsCurrency - NonDeliverySum);

  edtzt.Text := CurrToStr(Invmc_DataModule.getInTransit(qry.FieldByName('品号').AsString));
  FreeAndNil(Invmc_DataModule);
end;

procedure TFormInvmc.N1Click(Sender: TObject);
VAR
  sSQL:String;
begin
  inherited;
   sSQL := 'SELECT MOCTA.TA001 AS 单别, MOCTA.TA002 AS 单号, MOCTA.TA003 AS 单据日期, ' +
    ...
    FormOpenSearch := TFormOpenSearch.Create(nil,sSQL,'工单单号','订单单号','MOCTA.TA002','MOCTA.TA027');
    FormOpenSearch.Caption := '未完工工单查询';
    FormOpenSearch.dbgrdh.Font.Size := 12;
    FormOpenSearch.qry.Open;
    FormOpenSearch.ShowModal;
    FormOpenSearch.Free;
...
procedure TFormInvmc.N2Click(Sender: TObject);
VAR
  sSQL:String;
begin
  inherited;
    ...
    FormOpenSearch := TFormOpenSearch.Create(nil,sSQL,'客户简称','订单单号','COPMA.MA002','COPTC.TC002');
    FormOpenSearch.Caption := '相关订单查询';
    FormOpenSearch.dbgrdh.Font.Size := 12;
    FormOpenSearch.qry.Open;
    FormOpenSearch.ShowModal;
...
procedure TFormInvmc.dbgrdhDrawColumnCell(Sender: TObject;
...
procedure TFormInvmc.btnSumClick(Sender: TObject);
var
  sSQL :string;
begin
  inherited;
  ...
  sSQL := sSQL + ' and (INVMC.MC007 >0 or INVMC.MC014>0) Group by MB002';
    ...
    FormOpenSearch := TFormOpenSearch.Create(nil,sSQL,'品名','库存数量','MB002','INVMC.MC007');
    FormOpenSearch.Caption := '牌号分类汇总';
    FormOpenSearch.dbgrdh.Font.Size := 12;
    FormOpenSearch.qry.Open;
    FormOpenSearch.ShowModal;
    FormOpenSearch.Free;
...
procedure TFormInvmc.btnCloseClick(Sender: TObject);
begin
  inherited;
  Close;
end;

procedure TFormInvmc.N5Click(Sender: TObject);
VAR
  sSQL: String;
begin
  inherited;
   sSQL := 'SELECT MF001 预测编号,MF003 品号, MF004 品名,MF005 规格,MF008 预测数量,MF009 已受订量 ' +
    ...
    FormOpenSearch := TFormOpenSearch.Create(nil,sSQL,'预测编号','预测品号','MF001','MF003');
    FormOpenSearch.Caption := '未完成销售预测查询';
    FormOpenSearch.dbgrdh.Font.Size := 12;
    FormOpenSearch.qry.Open;
    FormOpenSearch.ShowModal;
    FormOpenSearch.Free;
...
procedure TFormInvmc.N3Click(Sender: TObject);
var
  sSQL:string;
begin
  inherited;
   ...
   + 'MOCTB.TB012 材料品名,MOCTB.TB013 材料规格,' +
     ...
     +' WHERE  MOCTB.TB003  ='''
       +qry.FieldByName('品号').AsString + ''' and MOCTA.TA011 <>''Y'' AND  MOCTA.TA011 <>''y''';
    FormOpenSearch := TFormOpenSearch.Create(nil,sSQL,'工单单号','工单品号','MOCTA.TA002','MOCTA.TA006');
    FormOpenSearch.Caption := '领料未完成工单查询';
    FormOpenSearch.dbgrdh.Font.Size := 12;
    FormOpenSearch.qry.Open;
    FormOpenSearch.ShowModal;
    FormOpenSearch.Free;
...
procedure TFormInvmc.N4Click(Sender: TObject);
var
  sSQL:string;
begin
  inherited;
   ...
   + 'TD008 采购数量,TD015 已交数量,TD008-TD015 欠交数量 '
    ...
    FormOpenSearch := TFormOpenSearch.Create(nil,sSQL,'采购单别','采购单号','TD001','TD002');
    FormOpenSearch.Caption := '未完成采购单';
    FormOpenSearch.dbgrdh.Font.Size := 12;
    FormOpenSearch.qry.Open;
    FormOpenSearch.ShowModal;
    FormOpenSearch.Free;
